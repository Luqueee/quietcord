...
import {parseMarkdown, plainText} from './parse.js';
  ...
  tokens.forEach((t, i) => {
    ...
    switch (t.kind) {
      ...
      case 'italic':
        ...
        break;
      case 'code':
        nodes.push(<Text key={k} color="yellow">{t.value}</Text>);
        break;
      case 'codeblock':
        nodes.push(
          <Box
      ...
      case 'quote':
        ...
        break;
      case 'mention_user':
        nodes.push(<Text key={k} color="blue">@{t.display}</Text>);
        break;
...
export function renderContent(props: Props): React.ReactNode {
  const {content, resolver, deleted, edited, width = 80} = props;
  if (deleted) {
...
export {parseMarkdown, plainText};
