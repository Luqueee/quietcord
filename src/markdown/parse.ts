import type {MdToken, MentionResolver} from './types.js';
...
export function parseMarkdown(input: string, resolver: MentionResolver): MdToken[] {
  ...
  while ((m = re.exec(input)) !== null) {
    if (m.index > cursor) {
      out.push(...parseLines(input.slice(cursor, m.index), resolver));
    }
    out.push({kind: 'codeblock', lang: m[1] ?? '', value: (m[2] ?? '').replace(/\n$/, '')});
    cursor = m.index + m[0].length;
  }
  if (cursor < input.length) {
    out.push(...parseLines(input.slice(cursor), resolver));
  }
  return out;
}

function parseLines(input: string, resolver: MentionResolver): MdToken[] {
